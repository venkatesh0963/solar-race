...
- **Responsive Mobile HUD:** Fully supports touch devices with an on-screen SVG D-Pad for 4-way movement, and a built-in HUD camera toggle button.
...
- **UI Buttons**: Pause, Resume, Restart

### Touch (Mobile)
- **On-Screen D-Pad**: Use the left side of the screen to tap/hold Up, Down, Left, and Right.
- **Camera Button**: Tap the 🎥 icon in the top right to change camera views.
...
2. **Energy Cores**: Look for the glowing cyan crystalline rings floating in space. Collecting them restores 15% of your energy and grants bonus points.
3. **Meteors**: Rocky, dark grey and brown asteroids that hurtle *towards* you at high speeds. Hitting one instantly ends the run.

---

## 🛠️ Tech Stack

- **[Three.js](https://threejs.org/)**: The core WebGL engine used for rendering the 3D scene, models, lighting, and math (Vectors/Bounding Boxes).
- **[Vite](https://vitejs.dev/)**: Next-generation frontend tooling used for instant server start, lightning-fast hot module replacement (HMR), and optimized production builds.
- **Vanilla JavaScript (ES6+)**: No heavy frameworks like React or Vue. Pure object-oriented ES6 classes.
- **HTML5 & CSS3**: For the HUD, touch controls, and typography.
...
│   ├── Game.js          # The core game loop, renderer, lighting, and camera logic
...
- Add sound effects and ambient space background music.
- Implement shooting mechanics to blast meteors out of the way.
- Add particle explosion effects for crashes.
